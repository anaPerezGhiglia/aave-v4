...
export default defineConfig({
  ...
  test: {
    solidity: {
      ...
      // Hardhat 3.3.0+ supports inline config at FUNCTION level (#7355 closed); EDR 0.12.0-next.33
      // (Hardhat 3.5.0) also added inline isolate / evm_version (edr#1349 closed). However, this
      // project's directives are at CONTRACT level — contract-level inline config is still silently
      // ignored by Hardhat, only function-level is honored. Setting globally remains the workaround.
      // Safe: only affects vm.expectRevert on internal calls.
      allowInternalExpectRevert: true,

      // eip712Types — required for vm.eip712HashStruct / vm.eip712HashType cheatcodes
      // (added in EDR 0.12.0-next.32 / Hardhat 3.5.0). Unlike Foundry, which auto-discovers
      // structs from all compiled artifacts, Hardhat 3 requires explicit include globs pointing
  ...
  //   → Hardhat 3.3.0+ supports function-level inline config (#7355 closed); Hardhat 3.5.0 / EDR
  //   next.33 added inline isolate / evm_version at function level (edr#1349 closed). But this
  //   project's directives are all CONTRACT-LEVEL — still silently ignored. Contract-level inline
  //   config remains unsupported in Hardhat 3.
});
